...
public class ChatOrchestrator
{
    ...
    public async Task<(string reply, string service, string nextStepsUrl, float score, List<string> suggestions)> HandleChatAsync(string sessionId, string message)
    {
        ...
        // 3. Name introduction — "I am John" / "My name is Sarah" — friendly greeting, avoid Housing routing
        if (IsNameIntroduction(normMsg, out var introducedName))
        {
            var nameReply = string.IsNullOrWhiteSpace(introducedName)
                ? "Hello! What can I help you with today?"
                : $"Hello {introducedName}! What can I help you with today?";
            ...
            SaveConversation(sessionId, message, nameReply, "Unknown", "greeting", nameSuggestions);
            return (nameReply, "Unknown", "", 0, nameSuggestions);
        }

        // 4. Vague help requests — no service signal, respond with a service menu
    ...
    !LooksLikeRealWord(words[0]))
{
    ...
    // Words that indicate the message is NOT a pure name introduction.
    // If any of these appear in the candidate name portion, the guard is skipped
    // and normal service routing continues (e.g. "I am at risk of eviction" passes through).
    private static readonly HashSet<string> _nameGuardServiceWords = new(StringComparer.OrdinalIgnoreCase)
    {
        "council", "tax", "bin", "bins", "waste", "recycling",
        "benefit", "benefits", "housing", "homeless", "school", "schools",
        "planning", "library", "libraries", "support", "help", "problem",
        "issue", "eligible", "apply", "applying", "risk", "eviction", "evicted",
        "worried", "struggling", "need", "want", "behind", "debt", "arrears",
        "payment", "blue", "badge", "appointment", "contact", "claim",
        "disabled", "disability", "pip", "rent", "landlord", "tenant",
        "worried", "scared", "lost", "homeless", "hungry", "cold"
    };

    /// <summary>
    /// Returns true when the message is a plain name introduction with no service context,
    /// e.g. "I am John" or "My name is Sarah". Outputs the name (title-cased) for the reply.
    /// Messages like "I am at risk of eviction" are NOT matched because they contain
    /// service-domain words that appear in <see cref="_nameGuardServiceWords"/>.
    /// </summary>
    private static bool IsNameIntroduction(string normMsg, out string name)
    {
        name = "";
        if (string.IsNullOrWhiteSpace(normMsg)) return false;

        // Match "i am [1-3 words]"  or  "i'm [1-3 words]"  or  "my name is [1-3 words]"
        var match =
            Regex.Match(normMsg, @"^(?:i am|i'm|im)\s+(.+)$", RegexOptions.IgnoreCase) is { Success: true } m1 ? m1 :
            Regex.Match(normMsg, @"^my name is\s+(.+)$",       RegexOptions.IgnoreCase) is { Success: true } m2 ? m2 :
            null;
        ...
        // Reject if too many words — real names are 1-3 words
        if (words.Length > 3) return false;

        // Reject if any word is a service/situation keyword
        foreach (var word in words)
        {
            if (_nameGuardServiceWords.Contains(word))
                return false;
        }

        // Reject if any word looks like a common English verb/adjective (not a name)
        // This catches "I am fine", "I am good", "I am okay" etc.
        var notNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
        {
            "fine", "good", "ok", "okay", "well", "great", "bad", "sad", "happy",
            "here", "ready", "back", "new", "confused", "lost", "stuck"
        };
        foreach (var word in words)
        {
            if (notNames.Contains(word)) return false;
        }

        // Build a title-cased name for the reply
        name = string.Join(" ", words.Select(w => w.Length > 0
    ...
    private static bool IsBinFollowUpIntent(string msg)
{
    ...
    private static string GetSmallTalkReply(string normMsg)
    {
        ...
        var greetings = new[]
        {
            $"Good {timeOfDay}! I can help with Council Tax, bins and waste, benefits, housing, schools, and more. What would you like to know?",
            ...
            $"Hello! I can help with Council Tax, bin collections, benefits, and much more. What do you need?"
        };
    ...
    private static bool IsContextResetIntent(string msg)
{
    ...
    // Words that, if present after "I am / I'm", indicate the message describes a
    // situation rather than a name — and must NOT be caught by the identity guard.
    private static readonly HashSet<string> _situationWords = new(StringComparer.OrdinalIgnoreCase)
    {
        "at", "in", "of", "with", "not", "unable", "trying", "going", "having",
        "getting", "risk", "worried", "scared", "afraid", "struggling", "behind",
        "facing", "experiencing", "a", "an", "the", "no", "without", "here",
        "there", "looking", "seeking", "desperate", "evicted", "homeless",
        "unemployed", "disabled", "ill", "sick", "worried", "concerned"
    };

    private static bool IsIdentityOnlyInput(string msg)
    {
        if (string.IsNullOrWhiteSpace(msg)) return false;

        // Must start with an identity prefix
        var match = Regex.Match(msg.Trim(),
            @"^(i am|i'm|my name is)\s+(.+?)(\s*[.,]?\s*(i want help|i need help|help|hi|hello|hey|please))?$",
            RegexOptions.IgnoreCase);
        if (!match.Success) return false;

        var namePart = match.Groups[2].Value.Trim();
        var nameWords = namePart.Split(' ', StringSplitOptions.RemoveEmptyEntries);

        // A name is 1–3 words. More words = a sentence about a situation.
        if (nameWords.Length > 3) return false;

        // If the first word after "I am" is a situation/circumstance word, it's not a name
        if (nameWords.Length > 0 && _situationWords.Contains(nameWords[0])) return false;
    ...
    private static bool IsVagueHelpRequest(string normMsg)
    {
        ...
        // Short messages that are just "help" variants with no service noun
        var words = normMsg.Split(' ', StringSplitOptions.RemoveEmptyEntries);
        if (words.Length <= 5 &&
            (normMsg.StartsWith("i want help") || normMsg.StartsWith("i need help") ||
             normMsg.StartsWith("help me") || normMsg.StartsWith("please help")))
            return true;
...
private static bool IsEnterPostcodeIntent(string normMsg)
{
    ...
    private static bool IsUnrelatedToAppointmentFlow(string normMsg)
    {
        ...
        // If the message starts with a question word and contains a service keyword,
        // it's almost certainly a new question, not a date/time/name input.
        ...
        // Service-domain keywords indicate a new topic
        ...
        return startsWithQuestion && hasServiceKeyword;
    }
